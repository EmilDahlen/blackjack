...
class player:
    ...
    def count_cards(self): # bug: 4 + 7 + 10 + A = 21. When it should = 22
        card_count = int()
        ace_count = int()
...
class game:
    ...
    def write_player(self, player, i):
        self.file.write(player.name + ";" + str(player.money) + ";" + str(player.has_betted) +  ";" + str(player.has_had_round) + ";" + str(player.has_got_dealt)+ ";" + str(player.game_done))
    ...
    def play_game(self):
        ...
        self.dealer.cards = []
        rematch = []
        for mndex, player in enumerate(self.player_list):
            player.cards = []
            while True:
                if player.money <= 0:
                    break
